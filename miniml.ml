...
let repl () =
  ...
  while true do
    (try
        ...
        (* EVALuate it *)
        let resS = Ev.evaluate_s exp env in
        let resD = Ev.evaluate_d exp env in
        let resL = Ev.evaluate_l exp env in
        ...
        printf "s==> %s\n" (Ev.Env.value_to_string resS);
        printf "d==> %s\n" (Ev.Env.value_to_string resD);
        printf "l==> %s\n" (Ev.Env.value_to_string resL)
      with
